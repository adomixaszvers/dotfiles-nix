...
let
  ...
  rofi-powermenu = pkgs.writeShellScript "rofi-powermenu" ''
    PATH=$PATH:${lib.makeBinPath [ rofi pkgs.procps ]}
  ...
  rofi-windows = pkgs.writeShellScript "rofi-windows" ''
        PATH=${lib.makeBinPath (with pkgs; [ sway jq rofi ])}
...
in {
  ...
  wayland.windowManager.sway = {
    ...
    config = {
      ...
      startup = [{
        command =
          "cat /proc/acpi/button/lid/LID/state | awk '{print $2}' | grep -q 'closed' && swaymsg output eDP-1 disable";
        always = true;
      }];
      window = lib.mkMerge [
