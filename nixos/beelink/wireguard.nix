  ...
  networking = {
    ...
    wireguard = {
      ...
      interfaces.wg0 = {
        ...
        postSetup = ''
          ${lib.getExe pkgs.iptables} -t nat -A POSTROUTING -s 10.6.0.0/24 -o enp2s0 -j MASQUERADE
        '';

        # This undoes the above command
        postShutdown = ''
          ${lib.getExe pkgs.iptables} -t nat -D POSTROUTING -s 10.6.0.0/24 -o enp2s0 -j MASQUERADE
        '';
        peers = [
          {
