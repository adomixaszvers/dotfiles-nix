...
type = custom/script
exec = ${import ./scripts/check-all-updates.nix}
interval = 1000
...
type = custom/script
exec = ${import ./scripts/compton.nix}
click-left = ${import ./scripts/compton-toggle.nix}
interval = 5
format-foreground = ${colors.foreground}
format-background = ${colors.background}
...
[module/load-average]
type = custom/script
exec = ${pkgs.coreutils}/bin/uptime | ${pkgs.gnugrep}/bin/grep -ohe 'load average[s:][: ].*' | ${pkgs.gawk}/bin/awk '{ print $3" "$4" "$5"," }' | ${pkgs.gnused}/bin/sed 's/,//g'
interval = 100
...
tail = true
label = %output%
exec = ${import ./scripts/pavolume.nix} --listen
click-right = exec ${pkgs.pavucontrol}/bin/pavucontrol
click-left = ${import ./scripts/pavolume.nix} --togmute
scroll-up = ${import ./scripts/pavolume.nix} --up
scroll-down = ${import ./scripts/pavolume.nix} --down
...
type = custom/script
exec = ${import ./scripts/pub-ip.nix}
interval = 100
format-foreground = ${colors.foreground}
format-background = ${colors.background}
format-underline = #FFBB00
...
[module/release]
type = custom/script
exec = (${pkgs.lsb-release}/bin/lsb_release -d | ${pkgs.gawk}/bin/awk {'print $2'} ;echo " "; ${pkgs.lsb-release}/bin/lsb_release -r | ${pkgs.gawk}/bin/awk {'print $2'}) | tr -d '\n'
interval = 6000
...
type = custom/script
exec = ${import ./scripts/spotify1.nix}
interval = 1
...
[module/temperature2]
type = custom/script
exec = ${import ./scripts/tempcores.nix}
interval = 2
format-padding = 1
format-foreground = ${colors.foreground}
format-background = ${colors.background}
...
type = custom/script
exec = ${pkgs.coreutils}/bin/uptime | ${pkgs.gawk}/bin/awk -F, '{sub(".*up ",x,$1);print $1}'
interval = 100
label = Uptime : %output%
...
format-foreground = ${colors.foreground}
format-background = ${colors.background}
exec = ${import ./scripts/weather.nix}
tail = true
